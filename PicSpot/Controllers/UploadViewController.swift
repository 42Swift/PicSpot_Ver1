...
//    var creationDate: Date?
    override func viewDidLoad() {
        ...
        checkedImageView.backgroundColor = .blue
        checkedImageView.frame = CGRect(x: 10, y: UIScreen.main.bounds.size.width + 10, width: UIScreen.main.bounds.size.width - 100, height: UIScreen.main.bounds.size.width - 100)
        StorageImageController().downloadImage(checkedImageView, ImageWithName(userName: "haha", profile: false, image: nil, imageName: "2022-10-21 17:58:03"))
        self.view.addSubview(checkedImageView)
    }
...
extension MapViewController: PHPickerViewControllerDelegate {
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        if let assetId = results[0].assetIdentifier {
            // abort 라이브러리에 대한 권한을 주지 않았기에 abort가 발생되었고, 한참을 해맸고 해결이 되었다.
        ...
        if results[0].itemProvider.canLoadObject(ofClass: UIImage.self) {
            ...
            results[0].itemProvider.loadObject(ofClass: UIImage.self) { image, error in
                DispatchQueue.main.async {
                    ...
                    StorageImageController().uploadImage(imageData: ImageWithName(userName: "haha", profile: false, image: self.myImageView.image!, imageName: nil))
                }
            }
        }
    }
